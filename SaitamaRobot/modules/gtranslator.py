...
def totranslate(update: Update, context: CallbackContext):
    ...
    try:
        if msg.reply_to_message:
            ...
            if source_lang is None:
                ...
                return message.reply_text(
                    f"Translated from *<b>{detection.lang}</b> to <b>{dest_lang}</b>:\n\n◣<code>{tekstr.text}</code>",
                    parse_mode=ParseMode.HTML,
                )
            else:
                tekstr = trl.translate(text, dest=dest_lang, src=source_lang)
                message.reply_text(
                    f"Translated from <b>{source_lang}</b> to <b>{dest_lang}</b>:\n\n◣<code>{tekstr.text}</code>",
                    parse_mode=ParseMode.HTML,
                )
        else:
        ...
        update.effective_message.reply_text(
            ...
            "See [List of Language Codes](t.me/ShinobuSupport/27) for a list of language codes.",
            parse_mode="markdown",
